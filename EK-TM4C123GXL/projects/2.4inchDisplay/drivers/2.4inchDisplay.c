...
// Hardware-timer blocking delays (delay_init/delay_ms/delay_us).
#include "delay.h"
...
void display_data_mode(void){
    // DC high to put the Display in data mode //
...
void display_init(void){
    ...
    // timer must be claimed first (harmless if main already called this).
...
void display_draw_char(uint16_t x, uint16_t y,char ch,uint16_t fgColor, uint16_t bgColor){
    ...
    display_enable();      // CS low
    display_data_mode();   // DC high — pixel stream
    ...
    // the next display_set_window() call. set_window sends SPI commands
    // and if SSI is still shifting pixels you corrupt the pixel stream.
    while (MAP_uDMAChannelSizeGet(UDMA_CH15_SSI3TX | UDMA_PRI_SELECT));
    while (SSIBusy(SSI3_BASE));

    display_disable();     // CS high
...
uint8_t touch_request_coords(void){
    ...
    if(MAP_GPIOPinRead(GPIO_PORTB_BASE, GPIO_PIN_2)) return 0;

    // Reading the controller makes PENIRQ pulse, which would otherwise queue a
    // spurious touch interrupt and keep touch_asserted stuck set. Mask PB2 for
    ...
    // invert each mapped result. The cal_* limits come from touch_calibration();
    // swap the channel->axis assignment if the panel is mounted rotated 90 degrees.
...
static void touch_cal_wait_release(void){
    uint16_t stable = 0;
    while(stable < 50){
        stable = MAP_GPIOPinRead(GPIO_PORTB_BASE, GPIO_PIN_2) ? stable + 1 : 0;
        delay_ms(1);
    }
    touch_pressed();
}
...
void touch_int_handler(void){
    // Read interrupt status from port B.
...
void touch_spi_config(void){
    ...
    MAP_SSIConfigSetExpClk(SSI0_BASE, MAP_SysCtlClockGet(), SSI_FRF_MOTO_MODE_0,SSI_MODE_MASTER,2000000, 16);
